...
def behavior_map(latest: pd.DataFrame, title: str = "Bản đồ hành vi VN30") -> go.Figure:
    ...
    for cluster_id in sorted(frame["Cluster"].astype(int).unique()):
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate=("Mã: %{customdata[0]}<br>Nhóm: %{customdata[1]}<br>"
                           "Độ chắc chắn: %{customdata[2]:.2f}<br>Hoạt động dòng tiền: %{customdata[3]:.2f}<extra></extra>"),
        ))
    ...
    if not centroids.empty:
        fig.add_trace(go.Scatter(
            ...
            text=[f"Tâm nhóm {int(c) + 1}" for c in centroids["Cluster"]], textposition="bottom center",
            name="Tâm nhóm", marker=dict(symbol="x", size=14, color="#111111", line=dict(width=2)),
            hovertemplate="%{text}<br>Vị trí tương đối: %{x:.2f}, %{y:.2f}<extra></extra>",
        ))
    ...
    fig.update_layout(
        ...
        xaxis_title="Sức mạnh tương đối",
...
def migration_heatmap(migration: pd.DataFrame, change_table: pd.DataFrame | None = None) -> go.Figure:
    ...
    fig = go.Figure(go.Heatmap(
        ...
        zmin=0, zmax=3, connectgaps=False, colorbar=dict(title="Nhóm hành vi", tickvals=[0, 1, 2, 3], ticktext=["Nhóm 1", "Nhóm 2", "Nhóm 3", "Nhóm 4"]),
